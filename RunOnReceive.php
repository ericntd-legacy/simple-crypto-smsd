...
if (!empty($smsMsg)) {
  ...
  if (count($parts)==2) {
    ...
    if (strcmp($parts[0], "gmstelehealth")==0) {
      ...
      $decrypted = decrypt(base64_decode($parts[1]), 128); // MyKeyUtils.php >> decrypt()
			
			$output .= "decrypted measurement: $decrypted \n";
			
		} else {
			$output .= "message does not contain a measurement, not sure what to do \n";
		}
	} else {
		$output .= "message does not have 2 parts - malformed \n";
        }
} else {
